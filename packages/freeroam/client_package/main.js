...
const pois = [];
jcmp.events.AddRemoteCallable('freeroam_create_poi', data => {
    const args = JSON.parse(data);

    // TODO(aaronlad): once https://gitlab.nanos.io/jc3mp/client/commit/289810f0905ca80a1e0635d78c3b72e151289d71 is fixed on the release stuff
    // move the poi text back into the constructor argument!
    const poi = new POI(args.type, new Vector3f(parseFloat(args.x), parseFloat(args.y), parseFloat(args.z)));
...
jcmp.events.AddRemoteCallable('freeroam_destroy_poi', uid => {
    if (typeof pois[uid] !== 'undefined') {
        pois[uid].Destroy();
        delete pois[uid];
    }
});
